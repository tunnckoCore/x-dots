...
storage.load().then(() => {
  _options.options.forEach(item => {
    formEl.innerHTML += (0, _components2.default)(_extends({}, item, {
      value: storage.get(item.name)
    }));
  });
});

formEl.addEventListener('change', (_ref) => {
...
exports.default = function (_ref) {
  ...
  if (type === 'checkbox') {
    return (0, _checkbox2.default)(rest);
  } else if (type === 'text' || type === 'password') {
    return (0, _input2.default)(_extends({ type }, rest));
  }
};
...
exports.default = (_ref) => {
  ...
  var val = value === undefined ? defaultValue : value;
  var note = description ? `<p class="note">${description}</p>` : '';
